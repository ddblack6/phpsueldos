...
if (!empty($testibase)) {
	...
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($db->PConnect("192.168.0.99:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", ""))
...
if (!empty($testvfp)) { // ODBC
	...
	if ( $db->PConnect('192.168.0.99','DRIVER={Microsoft Visual FoxPro Driver};SOURCETYPE=DBF;SOURCEDB=d:\inetpub\adodb;EXCLUSIVE=NO;')) {
	print "<h1>Connecting $db->databaseType...</h1>";flush();
	testdb($db,"create table d:\\inetpub\\adodb\\ADOXYZ (id int, firstname char(24), lastname char(24),created date)");
...
// REQUIRES MySQL server at 192.168.0.99 with database 'test'
if (!empty($testmysql)) { // MYSQL


	if (PHP_VERSION >= 5 || $_SERVER['HTTP_HOST'] == '192.168.0.99') $server = '192.168.0.99';
	...
	if (true || $db->PConnect($server, "root", "", "northwind")) {
		...
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) Type=InnoDB");
	} else print "ERROR: MySQL test requires a MySQL server on 192.168.0.99, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
}
...
if (!empty($testmysqlodbc)) { // MYSQL
	...
	$db->hasTransactions = false;
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($_SERVER['HTTP_HOST'] == '192.168.0.99') $server = '192.168.0.99';
	else $server = "mangrove";
...
if (!empty($testproxy)){
	...
	if ($db->PConnect('http://192.168.0.99/php/phplens/adodb/server.php'))
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb");
	else print "ERROR: MySQL test requires a MySQL server on 192.168.0.99, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testdb2)) {
	if (PHP_VERSION>=5.1) {
		...
		#$dsn = "driver={IBM db2 odbc DRIVER};Database=test;hostname=192.168.0.99;port=50000;protocol=TCPIP; uid=natsoft; pwd=guest";
		if ($db->Connect('192.168.0.99','natsoft','guest','test')) {
			testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
...
echo "<hr />";
flush();
	$dsn = "driver={IBM db2 odbc DRIVER};Database=sample;hostname=192.168.0.99;port=50000;protocol=TCPIP; uid=root; pwd=natsoft";
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
	...
	//$db->debug=1;
	$myDSN="SERVER=192.168.0.99;DATABASE=northwind;Trusted_Connection=yes";
...
if (extension_loaded('odbtp') && !empty($testmssql)) { // MS SQL Server via ODBC
	...
	if ($db->PConnect('192.168.0.99',$dsn, "", ""))  {
		print "<h1>Connecting $db->databaseType...</h1>";
		testdb($db,"create table ADOXYZ (id int, firstname char(24) null, lastname char(24) null,created datetime null)");
	}
	else print "ERROR: MSSQL test 1 requires a MS SQL 7 server setup with DSN setup";
