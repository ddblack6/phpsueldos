    ...
    <body>
          ...
          if  (empty($_POST['txtDescuentos'])){$descripcion=0;}else{ $descripcion = $_POST['txtDescuentos'];}
           ...
           if ($var==1){
			...
			alert("Este Descuento ya existe. Intente Ingresar otro Tipo Descuento..");
			 window.location="http://192.168.0.99/web/phpsueldos2015/userloget/FrmTipoDescuentoNuevo.php";
			 </script>';
                            ...
                            $query = '';
                            $var=0;
                            header("Refresh:0; url=http://192.168.0.99/web/phpsueldos2015/userloget/FrmTipoDescuentoNuevo.php");
                            }
         }
        ...
        elseif ($var==2){
            conexionlocal();
                ...
                //ejecucion del query
                $ejecucion = pg_query($query)or die('Error al realizar la carga');
                header("Refresh:0; url=http://192.168.0.99/web/phpsueldos2015/userloget/FrmTipoDescuentoModif.php");
            }
       //
        ?>
    </body>
